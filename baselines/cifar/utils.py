...
def load_input_fn(split,
  ...
  def input_fn(ctx=None):
    """Returns a locally sharded (i.e., per-core) dataset batch."""
    if proportion == 1.0 or split == tfds.Split.TEST:
      dataset = tfds.load(name, data_dir=data_dir, split=split, as_supervised=True, try_gcs=True)
    elif proportion>=0.0:
      new_name = '{}:3.*.*'.format(name)
      new_split = 'train[:{}%]'.format(int(100 * proportion))
      dataset = tfds.load(new_name, data_dir=data_dir, split=new_split, as_supervised=True, try_gcs=True)
    else:
      new_name = '{}:3.*.*'.format(name)
      new_split = 'train[{}%:]'.format(int(100 * proportion))
      dataset = tfds.load(new_name, data_dir=data_dir, split=new_split, as_supervised=True, try_gcs=True)
    ...
    if name!= 'mnist' and name!='fashion_mnist':
        dataset = dataset.map(preprocess,
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
    else:
        dataset = dataset.map(preprocess2,
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def load_input_fn_local(split,
  ...
  def input_fn(ctx=None):
    """Returns a locally sharded (i.e., per-core) dataset batch."""
    if proportion == 1.0:
      dataset = tfds.load(name, split=split, as_supervised=True, try_gcs=True)
    else:
      new_name = '{}:3.*.*'.format(name)
      if split == tfds.Split.TRAIN:
        new_split = 'train[:{}%]'.format(int(100 * proportion))
      ...
      dataset = tfds.load(new_name, split=new_split, as_supervised=True, try_gcs=True)
    if split == tfds.Split.TRAIN:
      dataset = dataset.shuffle(buffer_size=dataset_size).repeat()
...
def restore_checkpoint(checkpoints_path, FLAGS, FLAG_hash, model, optimizer, steps_per_epoch):
    checkpoint = tf.train.Checkpoint(model=model[0], optimizer=optimizer)
    latest_checkpoint = tf.train.latest_checkpoint(
        fix_path(f'{checkpoints_path}/{FLAG_hash}/', True)
    )
    initial_epoch = 0
    if FLAGS.checkpoint_interval > -1 and latest_checkpoint:
...
def save_checkpoint(checkpoint, checkpoints_path, FLAG_hash):
    checkpoint_name = checkpoint.save(
        os.path.join(fix_path(f'{checkpoints_path}/{FLAG_hash}/', is_folder=True), 'step'))
